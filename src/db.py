...
async def del_note(note_id: int):
    async with conn_pool.acquire() as connection:
        result = await connection.execute('DELETE FROM notes WHERE id = $1', note_id)
        return result
...
async def edit_notes(note_id: int, title: str, content: str):
    ...
    async with conn_pool.acquire() as connection:
        if title != "" and content != "":
            result = await connection.execute('UPDATE notes SET title = $1, content = $2, date_created = $3 WHERE id = $4', title, content, update_date,note_id)
        elif title == "":
            result = await connection.execute('UPDATE notes SET content = $1, date_created = $2 WHERE id = $3', content, update_date, note_id)
        elif content == "":
            result = await connection.execute('UPDATE notes SET title = $1, date_created = $2 WHERE id = $3', title, update_date, note_id)
        else:
            result = ['Cannot write with empty data']
        
        return result

# Returns a note with specific
async def note(note_id: int):
    async with conn_pool.acquire() as connection:
 
        record = await connection.fetch('SELECT * FROM notes WHERE id = $1', note_id)
        record = record[0]     
